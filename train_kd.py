...
class ImageTestDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        images = ops.ms_resize(image, scales=(1.0, 1.5), base_h=base_h, base_w=base_w)

        image_m = torch.from_numpy(images[0]).div(255).permute(2, 0, 1)
        image_l = torch.from_numpy(images[1]).div(255).permute(2, 0, 1)
        return dict(
            data={"image_m": image_m, "image_l": image_l},
...
class ImageTrainDataset(data.Dataset):
    def __init__(self, dataset_infos: dict, shape: dict):
        ...
        for dataset_name, dataset_info in dataset_infos.items():
            ...
            valid_names = sorted(
                set(image_names).intersection(mask_names))
    ...
    def __getitem__(self, index):
        ...
        mask = ops.resize(mask, height=base_h, width=base_w)  # 改gt图大小
...
def test(model, cfg, is_valid = False):
    if is_valid:
        test_wrapper = Evaluator(device=cfg.device, metric_names=("mae", ), clip_range=cfg.test.clip_range)
    else:
    ...
    for te_name in cfg.test.data.names:
        if is_valid and te_name != "cod10k_te":
            continue
        ...
        seg_results_str = ", ".join([f"{k}: {v:.06f}" for k, v in seg_results.items()])
        LOGGER.info(f"({te_name}): {py_utils.mapping_to_str(te_info)}\n{seg_results_str}")

        if is_valid and te_name == "cod10k_te":
            return seg_results["mae"]
...
def val(model, cfg):
    ...
    te_info = cfg.dataset_infos["cod10k_te"]
    te_dataset = ImageTestDataset(dataset_info=te_info, shape=cfg.test.data.shape)
    te_loader = data.DataLoader(
        dataset=te_dataset, batch_size=cfg.test.batch_size, num_workers=cfg.test.num_workers, pin_memory=True
    )
    ...
    mae_sum = 0
    for batch in tqdm(te_loader, total=len(te_loader), ncols=79, desc="[Val]"):
        batch_images = pt_utils.to_device(batch["data"], device="cuda")
        logits = model(data=batch_images)  # B,1,H,W
        probs = logits.sigmoid().squeeze(1).cpu().detach().numpy()
        probs = probs - probs.min()
        probs = probs / (probs.max() + 1e-8)
        ...
        for pred_idx, pred in enumerate(probs):
            ...
            mae_sum += np.sum(np.abs(pred - mask_array)) * 1.0 / (mask_h * mask_w)

    mae_avg = mae_sum / len(te_dataset)
    model.train()
    return mae_avg
    ...
    for epoch in range(counter.num_epochs):
        ...
        for batch_idx, batch in enumerate(tr_loader):
            ...
            if cfg.log_interval > 0 and (
                ...
                LOGGER.info(f"{eta_string}({gpu_mem}) | {progress} | {loss_info} ")
        ...
        curr_mae = test(model, cfg, is_valid=True)
        if epoch == 0:
            best_epoch = epoch
            best_mae = curr_mae
        else:
            if curr_mae < best_mae:
                best_epoch = epoch
                best_mae = curr_mae
                io.save_weight(model=model, save_path=(cfg.path.final_state_net).replace("state_final", "best"))
        LOGGER.info(f"Best epoch: {best_epoch}, best mae: {best_mae:.6f}, curr mae: {curr_mae:.6f}")
...
def parse_cfg():
    ...
    parser.add_argument("--data-cfg", type=str, default="./Dataset/dataset.yaml")
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default=["sm", "wfm", "mae", "fmeasure", "em"],
...
def main():
    ...
    teacher_model = model_zoo.__dict__.get("CFFKDNet_PVTB4")
    teacher = teacher_model(num_frames=1, pretrained=True)
    teacher.load_state_dict(torch.load('./best_b4_teacher.pth'),False)
    ...
    model_class.set_teacher(teacher)
    ...
    LOGGER.info(f"Parameter Reduction: {(teacher_params - student_params) / teacher_params * 100:.2f}%")
    if not cfg.evaluate:
        train(model=model, cfg=cfg)

    if cfg.evaluate or cfg.has_test:
        model_class.set_teacher(None)
        test_model = model_class(num_frames=1, pretrained=cfg.pretrained)
        test_model = test_model.to(cfg.device)
        io.load_weight(model=test_model, load_path=(cfg.path.final_state_net).replace("state_final", "best"))
        test(model=test_model, cfg=cfg)
