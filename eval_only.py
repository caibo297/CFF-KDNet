...
def main():
    ...
    params = sum(p.numel() for p in model.parameters(recurse=True))
    LOGGER.info(f"Number of Parameters: {params / 1e6:.2f}M")
    from thop import profile
    # 创建一个假的输入张量来计算FLOPs
    input_tensor = torch.randn(1, 3, 384, 384).to(cfg.device)
    input_dict = {"image": input_tensor, "image_m": input_tensor, "image_l": input_tensor}  # 假设模型需要这两个键

    # 使用正确的输入格式计算FLOPs
    flops, params = profile(model, inputs=(input_dict,))
    LOGGER.info(f"thop估算Flops: {flops / 1e9:.2f}G")
    LOGGER.info(f"thop估算参数量: {params / 1e6:.2f}M")
    if not cfg.evaluate:
